...
func (d *db) FindAll(ctx context.Context) (u []user.User, err error) {
	...
	if cursor.Err() != nil {
		return u, fmt.Errorf("failed to find all users by due to %v", err)
	}
...
func (d *db) FindOne(ctx context.Context, id string) (u user.User, err error) {
	...
	if result.Err() != nil {
		...
		return u, fmt.Errorf("failed to find one user by id %s due to %v", id, err)
	}

	if err := result.Decode(&u); err != nil {
		return u, fmt.Errorf("failed to decode user (id: %s)from db  due to %v", id, err)
	}
	return u, nil
}
...
func (d *db) Create(ctx context.Context, user user.User) (string, error) {
	...
	d.logger.Debug("convert insertingID to ObjectId")
...
func (d *db) Update(ctx context.Context, user user.User) error {
	...
	d.logger.Tracef("Matched %d documents, modified %d documents", result.MatchedCount, result.ModifiedCount)
